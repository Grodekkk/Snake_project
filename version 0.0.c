...
{
	DrawSurface(screen, SNAKE->snaker, SNAKE->x, SNAKE->y);
}
...
void moveSnake(worm* SNAKE)
{
	...
	{
		case 'w':
			if (SNAKE->y == 75)  //100, 125 CHANGE INTO BORDER CONSTS
			{
				SNAKE->y = SCREEN_HEIGHT - 50;
				break;
			}
			SNAKE->y -= 25;
			break;
		...
		case 'a':
			if (SNAKE->x ==125)  //100, 125 CHANGE INTO BORDER CONSTS
			{
				SNAKE->x = SCREEN_WIDTH - 100;
				break;
			}
			SNAKE->x -= 25;
			break;
		...
		case 's':
			if (SNAKE->y == SCREEN_HEIGHT - 50)  //100, 125 CHANGE INTO BORDER CONSTS
			{
				SNAKE->y = 75;
				break;
			}
		...
		case 'd':
			if (SNAKE->x == SCREEN_WIDTH - 100)  //100, 125 CHANGE INTO BORDER CONSTS
			{
				SNAKE->x = 125;
				break;
			}
...
//	                                 &window, &renderer);
	...
	while(!quit) {
		...
		sprintf(text, "Debbuging info: snakeWorldTime = %.1lf s, Snakedir  %.0c ", snakeWorldTime, SNAKE.dir);
