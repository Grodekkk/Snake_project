...
void initializeGameData(game* GAMEDATA, sdl* GAME)
{
	...
	GAMEDATA->snakeSpeed = SNAKE_SPEED;
...
{
	DrawRectangle(GAME->screen, 4, 4, SCREEN_WIDTH - 8, 36, COLOR->czerwony, COLOR->niebieski);
	sprintf(GAMEDATA->text, "Time elapsed: %.1lf s, POINTS: %d", GAMEDATA->worldTime, SNAKE->points);
	DrawString(GAME->screen, GAME->screen->w / 2 - strlen(GAMEDATA->text) * 8 / 2, 10, GAMEDATA->text, GAME->charset);
	sprintf(GAMEDATA->text, "Esc - wyjscie, n - new game");
	DrawString(GAME->screen, GAME->screen->w / 2 - strlen(GAMEDATA->text) * 8 / 2, 26, GAMEDATA->text, GAME->charset);
}
...
int main(int argc, char** argv)
{
	...
	while (!GAMEDATA.quit) {
		...
		GAMEDATA.snakeWorldTime += GAMEDATA.delta;
		if (GAMEDATA.snakeWorldTime >= GAMEDATA.snakeSpeed)
		{
			GAMEDATA.tickPassed = 1;
			GAMEDATA.snakeWorldTime = 0;
		}
