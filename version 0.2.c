...
void handleGameOver(worm* SNAKE, SDL_Surface *screen, colors *COLOR)
{
	char decision = 'x';
	DrawRectangle(screen, 110, 60, SCREEN_WIDTH - 195, SCREEN_HEIGHT - 95, COLOR->zielony, COLOR->niebieski);
	//while (decision == 'x')
	//{
		//infinite loop for now must be player decision what to do
	//}
}

void checkCollision(worm* SNAKE)
{
	for (int i = 1; i < SNAKE_SIZE; i++)
	{
...
//	                                 &window, &renderer);
	...
	initaializeSnake(&SNAKE);
	initializeColors(&COLOR, screen);

	t1 = SDL_GetTicks();

	//this can be moved into a function(init), maybe into a structure(game)
	frames = 0;
	fpsTimer = 0;
	fps = 0;
	quit = 0;
	...
	while(!quit) {
		if (SNAKE.crash == 0)
		{
			...
			SDL_FillRect(screen, NULL, COLOR.czarny);
			...
			// info text at top
			DrawRectangle(screen, 4, 4, SCREEN_WIDTH - 8, 36, COLOR.czerwony, COLOR.niebieski);
			sprintf(text, "Debbug info: snakeWorldTime = %.1lf s, snake[0]  %.0d POINTS: %d", snakeWorldTime, SNAKE.x[0], SNAKE.points);
			DrawString(screen, screen->w / 2 - strlen(text) * 8 / 2, 10, text, charset);
			sprintf(text, "Esc - wyjscie, n - new game");
			DrawString(screen, screen->w / 2 - strlen(text) * 8 / 2, 26, text, charset);
		}
		else //this is made this way for screen to keep updating and not crashing
		{
			DrawRectangle(screen, 4, 50, SCREEN_WIDTH - 8, 36, COLOR.czerwony, COLOR.niebieski);
			sprintf(text, "You lost and scored %d points!", snakeWorldTime, SNAKE.x[0], SNAKE.points);
			DrawString(screen, screen->w / 2 - strlen(text) * 8 / 2, 55, text, charset);
			sprintf(text, "Press n to go into a new game, esc to exit");
