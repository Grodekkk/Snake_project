...
typedef struct {
	...
	int newCellNumber;												//used in lenghtening, number of snake new body part [NOT USED, CHECK EVERY OTHER VARIABLE LIKE THIS]
...
void moveSnake(worm* SNAKE, dot *BLUE)
{
	...
	checkBlueDot(BLUE, SNAKE);

	if (SNAKE->lengheningReq == 1)
	{
...
void handleSnake(worm* SNAKE, colors* COLOR, sdl* GAME, game* GAMEDATA, dot *BLUE)
{
	if (GAMEDATA->tickPassed == 1)			//in void updateGamedata
	{
		moveSnake(SNAKE, BLUE);					//how to handle game speed?    ONE FUNCTION HANDLESNAKE?
		GAMEDATA->tickPassed = 0;
	}
...
void updateGameData(game* GAMEDATA)
{
	...
	GAMEDATA->redDotWorldTime += GAMEDATA->delta;
...
{
	checkIfSnakeEaten(BLUE, SNAKE);//CHECK IF SNAKE EATEN WAS HERE [FUNCTION]
...
void drawRedDot(bonus *RED, sdl *GAME)
{
	if (RED->onScreen == 0)
	{
...
void checkIfRedDotEaten(bonus* RED, worm* SNAKE, game *GAMEDATA)
{
	...
	if ((SNAKE->x[0] == RED->x) && (SNAKE->y[0] == RED->y))
	{
		if (rand() % 2 == 0)
		{
			GAMEDATA->snakeSpeed += RED_DOT_SPEED_DEDUCTION;
		}
		else
		{
			if (SNAKE->lenght - RED_DOT_LENGHT_DEDUCTION >= 1)
			{
...
void handleRedDot(bonus* RED, sdl* GAME, game *GAMEDATA, worm *SNAKE)
{
...
int main(int argc, char** argv)
{
	...
	while (!GAMEDATA.quit)
	{
		...
		handleBlueDot(&BLUE, &SNAKE, &GAME); //switched kolejnosc with handle snake
		handleRedDot(&RED, &GAME, &GAMEDATA, &SNAKE);
		handleSnake(&SNAKE, &COLOR, &GAME, &GAMEDATA, &BLUE);
		...
		while (SDL_PollEvent(&GAME.event))
		{
			...
			case SDL_KEYDOWN:														//button is pressed
				...
				else if (GAME.event.key.keysym.sym == SDLK_l ) //for checking delete this if statement later
				{
					SNAKE.lengheningReq = 1;
					break;
				}
